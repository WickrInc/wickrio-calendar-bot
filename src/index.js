  ...
  if (code) {
    oAuth2Client.getToken(code, (err, token) => {
      if (err) {
        WickrIOAPI.cmdSendRoomMessage(
          vGroupID,
          'Error retrieving access token, please try again or contact your network administrator.'
        )
    ...
    calendar.events.list(
      ...
      (err, res) => {
        if (err) {
          WickrIOAPI.cmdSendRoomMessage(
            vGroupID,
            'Sorry, I was not able create this Google Calendar event: ' +
              err +
          ...
          if (err.Error === 'invalid_grant') user.token = ''
          return
        }
  ...
  calendar.events.insert(
    ...
    function (err, response) {
      if (err) {
        WickrIOAPI.cmdSendRoomMessage(
          vGroupID,
          'Sorry, I was not able create this Google Calendar event, please try again or contact your network administrator.'
        )
        console.log(
          'There was an error contacting the Calendar service: ' + err
        )
      ...
      WickrIOAPI.cmdSendRoomMessage(
        vGroupID,
        'By the way, would you also like to set up Wickr in-app notifications for this event?'
      )
    }
  )
}
  ...
  calendar.events.update(
    ...
    function (err, response) {
      if (err) {
        WickrIOAPI.cmdSendRoomMessage(
          vGroupID,
          'Sorry, I was not able modify this Google Calendar event: ' + err
        )
        console.log(
          'There was an error contacting the Calendar service: ' + err
        )
    ...
    try {
      const job = new CronJob(
        ...
        function () {
          WickrIOAPI.cmdSendRoomMessage(
            wickrUser.vGroupID,
            'Google Calendar event titled: ' +
              event.title +
              ' starts in ' +
              event.wickrReminderTime
          )
        }.bind(null, event),
        null,
        true,
  ...
  try {
    ...
    if (command === '/list') {
      ...
      if (!user || isNaN(argument)) {
        return WickrIOAPI.cmdSendRoomMessage(
          vGroupID,
          'Please run the /list command first and send the /modify command again with an event number from the list (ex: /modify 3)'
        )
      }
      user.event = {}
      user.index = 1
      ...
      if (current < 16 && current > 0) {
        if (current === 1) {
          ...
          if (message !== 'skip') user.event.location = message
          else {
            user.event.location = 'none'
          }
        } else if (current === 3) {
          if (message !== 'skip') user.event.description = message
          else {
          ...
          if (!moment(message, 'MM-DD-YYYY').isValid()) {
            return console.log(
              WickrIOAPI.cmdSendRoomMessage(
                vGroupID,
                'Invalid date entry, please use the following format MM/DD/YYYY'
              )
            )
          }
          ...
          if (response) {
            ...
            if (
              !moment(message, 'h:mm A').isValid() ||
              !moment(message, 'HH:mm:ss').isValid()
            ) {
              return console.log(
                WickrIOAPI.cmdSendRoomMessage(
                  vGroupID,
                  'Invalid start time entry, please use a 12-hour or 24-hour format(ex: 5AM or 05:00)'
                )
              )
            }
          ...
          if (
            ...
            return console.log(
              WickrIOAPI.cmdSendRoomMessage(
                vGroupID,
                'Invalid end time entry, please use a 12-hour or 24-hour format(ex: 7:45PM or 19:45)'
              )
            )
          }
                ...
                return console.log(
                  WickrIOAPI.cmdSendRoomMessage(
                    ...
                    emails[i].replace(/\s/g, '') +
                      ' is an invalid email address, please try again'
                  )
                )
              } else {
                attendees.push({
          ...
          if (!response) {
            return console.log(
              WickrIOAPI.cmdSendRoomMessage(
                vGroupID,
                'Invalid choice, please enter one of the following: email, popup, both or none'
              )
            )
          }
          if (choice === 'none') {
          ...
          Object.keys(event).forEach(function (key, index) {
            ...
            if (fieldDescriptions[index] === undefined) return
          ...
          if (message === ('yes' || 'y')) {
            ...
            return console.log(
              WickrIOAPI.cmdSendRoomMessage(
                vGroupID,
                'What type of reminder would you like: personal, room or both?'
              )
            )
          } else {
            user.index = 0
          ...
          if (message === 'room' || message === 'both') {
            ...
            return console.log(
              WickrIOAPI.cmdSendRoomMessage(vGroupID, roomsMessage)
            )
          } else {
            user.index += 1
            return console.log(
